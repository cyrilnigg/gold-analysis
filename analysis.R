...
gold <- read.csv("/data/historical-gold-prices.csv")
summary(gold)

### Load gold data Doug created ###
gold <- read.csv('~/r/gold-prices/data/doug_gold3.csv')
summary(gold)
...
gold.training <- subset(gold, as.Date(Date) <= '2007-01-01')
gold.test <- subset(gold, as.Date(Date) > '2007-01-01')


##### BUILD LINEAR MODEL #####
lm.gold <- lm(GoldNextMonth ~ Gold + S.P + US.Dollar.Index + CPI, data=gold)
summary(lm.gold)
anova(lm.gold)
